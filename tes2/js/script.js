...
(function () {
  ...
  function isMobile(type) {
    ...
    const iPad = !!(
      reg.find((str) => str === "iPhone|iPad|iPod") &&
  ...
  ready(function () {
    ...
    if (isMobile() && navigator.userAgent.match(/Chrome/i)) {
      ...
      (function (width, height) {
        ...
        window.smartresize(function () {
          ...
          if (deviceSize.indexOf(windowHeight) < 0) windowHeight = deviceSize[window.innerWidth > windowHeight ? 1 : 0];
      ...
      $(document).on("add.cards", function (event) {
        ...
        if (
          ...
          outerFind(event.target, ".mbr-section--full-height").length
        )
    ...
    window.smartresize(function () {
      document.querySelectorAll(".mbr-embedded-video").forEach(function (el) {
        el.style.height =
          (
            (getWidth(el) * parseInt(el.getAttribute("height") || 315)) /
            parseInt(el.getAttribute("width") || 560)
          ).toFixed() + "px";
      });
    });
    if (isJQuery)
      $(document).on("add.cards", function (event) {
        ...
        if (
          document.querySelector("html").classList.contains("mbr-site-loaded") &&
          outerFind(event.target, "iframe").length
        )
          window.dispatchEvent(new CustomEvent("resize"));
      });
        ...
        if (parsedUrl && (/youtu\.?be/g.test(parsedUrl[3]) || /vimeo/g.test(parsedUrl[3]))) {
          ...
          if (parsedUrl && /youtu\.?be/g.test(parsedUrl[3])) {
            ...
            image.onload = function () {
              ...
              if (el.querySelector(".mbr-background-video-wrapper"))
                el.querySelector(".mbr-background-video-wrapper").remove();
              ...
              var videoResolution = {
                height:
                  imageResolution.height < sectionResolution.height ? imageResolution.height : sectionResolution.height,
                width:
                  imageResolution.width > sectionResolution.width ? imageResolution.width : sectionResolution.width,
              };
              ...
              player.on("playing", () => {
                player.replayFrom(1);
                if (player.getProgress() > 0) player._player.i.style.display = "block";
              });
  ...
  if (!isBuilder) {
      ...
      $(document).on("add.cards", function (event) {
        ...
        outerFind(event.target, ".mbr-social-likes").forEach(function (el) {
          el.addEventListener("counter.social-likes", function (event, service, counter) {
            if (counter > 999)
              event.target.querySelectorAll(".social-likes__counter").forEach(function (el) {
    ...
    ready(function () {
      ...
      if (isMobile()) {
        ...
        var animatedElements = Array.from(
        ...
        animatedElements = animatedElements.filter(function (el) {
          if (
            !el.parents().filter(function (parentElement) {
              if (
        ...
        function elCarouselItem(element) {
          if (
            element.parents(".carousel-item").some(function (x) {
          ...
          if (parentEl.querySelectorAll(".carousel-item.active .hidden.animate__animated").length) {
            ...
            if (
              element.parents().some(function (el) {
  ...
  if (document.querySelectorAll("nav.navbar").length) {
    ...
    if (document.querySelector(".mbr-after-navbar.mbr-fullscreen"))
      document.querySelector(".mbr-after-navbar.mbr-fullscreen").style.paddingTop = navHeight + "px";
  }
  ...
  function initTestimonialsCarousel(card) {
    ...
    if (target.querySelector(".carousel-controls"))
      target.querySelectorAll(".carousel-controls").forEach(function (el) {
        ...
        el.addEventListener("click", function () {
          var id = el.closest(".accordionStyles").getAttribute("id"),
            ...
            sign = el.querySelector("span.sign")
              ? el.querySelector("span.sign")
              : el.querySelector("span.mbr-iconfont");
            ...
            if (id.indexOf("toggle") != -1 || id.indexOf("accordion") != -1) {
              ...
              if (id.indexOf("accordion") != -1) {
                ...
                Array.from(accordion.children)
                  ...
                  .forEach(function (el) {
                    var icon = el.querySelector("span.sign")
                      ? el.querySelector("span.sign")
                      : el.querySelector("span.mbr-iconfont");
  ...
  function setGradient(section) {
    ...
    if (gradientElement) {
      ...
      if (
        !Array.from(section.querySelectorAll("svg")).some(function (el) {
  ...
  function setProgressElement(el, cardID, percent) {
    ...
    if (!card.getAttribute("style")) card.setAttribute("style", "-webkit-flex-basis: auto; flex-basis: auto;");
    ...
    el.setAttribute(
      ...
      `{ ${setGradient(
        el.closest("section")
      )} "percent": ${percent}, "size": 150, "colorCircle": "#f1f1f1", "stroke": 5, "colorSlice": "url(#${cardID})", "fontSize": "1.3rem", "number": false }`
    );
  ...
  function updateCircleProgress(section, paramName) {
    if (
      !section.classList.contains("circle-progress-section") ||
      !paramName.includes("progress") ||
      paramName == "progressCount"
    )
      return;
...
!(function () {
  ...
  if (!document.getElementById("top-1")) {
    ...
    a.innerHTML =
      '<a href="https://mobiri.se/a9">Web Creator</a> Mobirise v5.6.10 <a href="https://mobiri.se/a7">Website Creation</a>';
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const targetMenu = document.querySelector("#navbarSupportedContent");

  toggleBtn.addEventListener("click", function () {
    this.classList.toggle("open");
    // targetMenu.classList.toggle("show"); // якщо треба відкрити/закрити меню вручну
  });
});
